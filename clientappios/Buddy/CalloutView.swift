...
class CalloutView: UIView, MGLCalloutView {
    ...
    func presentCallout(from rect: CGRect, in view: UIView, constrainedTo constrainedView: UIView, animated: Bool) {
        ...
        if isCalloutTappable() {
            ...
            mainBody.addTarget(self, action: #selector(CalloutView.calloutTapped), for: .touchUpInside)
        } else {
    ...
    override func draw(_ rect: CGRect) {
        ...
        /*
    override func draw(_ rect: CGRect) {
        let arrowHeight = CGFloat(50.0)
        
        let context = UIGraphicsGetCurrentContext()
        let fillPath = UIBezierPath()
        fillPath.move(to: CGPoint(x: 0, y: self.bounds.origin.y + arrowHeight))
        fillPath.addLine(to: CGPoint(x: self.bounds.size.width/2-(arrowHeight/2), y: arrowHeight))
        fillPath.addLine(to: CGPoint(x: self.bounds.size.width/2, y: 0.0))
        fillPath.addLine(to: CGPoint(x: self.bounds.size.width/2+(arrowHeight/2), y: arrowHeight))
        fillPath.addLine(to: CGPoint(x: self.bounds.size.width, y: arrowHeight))
        fillPath.addLine(to: CGPoint(x: self.bounds.size.width, y: self.bounds.size.height))
        fillPath.addLine(to: CGPoint(x: 0, y: self.bounds.size.height))
        fillPath.close()
